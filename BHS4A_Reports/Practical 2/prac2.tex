...
The step response to a proportional constant and a integral constant of 10 is
shown by Figure \ref{fig:question_4_output_response_pi}
...
For a proportional and derivative controller the code used to produce the
proportional and integral controller can be modified as follows below to give
the response as shown by Figure \ref{fig:question_4_output_response_pd}\\
\noindent
\texttt{G = tf([R\*b+K\^2],[J\*L (L\*b+R\*J) R\*b+K]);}
\texttt{D = tf([Kd Kp],[1])}
\texttt{H = feedback(D \* G)}
...
Again the code can be modified to include an integral term as well to form the
PID controller substituting \texttt{D = tf([Kd Kp Ki],[1 0])} in the code
above. The response that is then produced is given by Figure
\ref{fig:question_4_output_response_pid}:
